 ...
 * serialized body) via each framework's own idiomatic mechanism - Fastify's `onSend` payload hook,
 * Elysia's `mapResponse`, Express's `res.json` wrap, Hono's drain-and-rebuild middleware shape, and
 * inline for the raw ceiling. The nifra counterpart lives in serve-node-nifra.ts (`body` arg).
 *
 *   node bench/http-realworld/serve-node.ts <fastify|hono|express|elysia|node-raw|*-body> <port>
 */
...
if (!Number.isInteger(port)) {
  throw new Error(
    "usage: node bench/http-realworld/serve-node.ts <fastify|hono|express|elysia|node-raw|*-body> <port>",
  )
}
...
if (framework === "fastify") {
...
} else if (framework === "fastify-body") {
  ...
  app.listen(port)
} else if (framework === "express-body") {
  const { default: express } = await import("express")
  const app = express()
  app.use(express.json())
  app.use((req, res, next) => {
  ...
  // Express's body-observing idiom: wrap res.json so the middleware sees the serialized body
  // (the same interception shape compression() uses on res.write/res.end).
  app.use((_req, res, next) => {
    res.json = ((value: unknown) => {
      const body = JSON.stringify(value)
      res.setHeader("content-type", "application/json")
      res.setHeader("x-body-hash", hash(body))
  ...
  server.listen(port)
} else if (framework === "node-raw-body") {
  // The body-hash ceiling: node:http, the hash computed inline at serialization time. No framework
  // means no tier to pay for - this is the floor the body-observing rows chase.
  const send = (
    res: ServerResponse,
    status: number,
    value: unknown,
  ...
  const server = createServer((req, res) => {
    const url = req.url ?? "/"
    if (!url.startsWith("/api/orders")) return send(res, 404, { error: "not_found" })
    const who = authOf(req.headers.authorization, req.headers.cookie)
    const requestId = (req.headers["x-request-id"] as string) ?? crypto.randomUUID()
    if (who === undefined) {
      return send(res, 401, { ok: false, error: "unauthorized" }, { "x-request-id": requestId })
    }
    if (req.method === "GET") {
      const search = new URL(url, "http://localhost").searchParams
      const limit = search.get("limit")
      if (limit === null) return send(res, 400, { error: "invalid" }, { "x-request-id": requestId })
      return send(
        res,
        200,
        {
    ...
    if (req.method === "POST") {
      ...
      req.on("end", () => {
        ...
        try {
          ...
          return send(res, 400, { error: "bad_json" }, { "x-request-id": requestId })
        }
        if (!isOrderBody(parsed)) {
          return send(res, 400, { error: "invalid" }, { "x-request-id": requestId })
        }
        send(
          res,
          200,
    ...
    send(res, 404, { error: "not_found" }, { "x-request-id": requestId })
  })
  server.listen(port)
} else {
