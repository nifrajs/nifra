/**
 * Portable request-deadline mechanics owned by core.
 *
 ...
 * add time back to an in-flight request. Policy stays out of this primitive.
 */
...
const assertFiniteNonNegative = (name: string, value: number): void => {
  if (!Number.isFinite(value) || value < 0) {
    throw new RangeError(`@nifrajs/core/budget: ${name} must be a finite non-negative number`)
  }
}
...
const assertDeadline = (deadline: number): void => {
  if (!Number.isSafeInteger(deadline) || deadline <= 0) {
    throw new RangeError(
      "@nifrajs/core/budget: deadline must be a positive safe-integer epoch-ms value",
    )
  }
}
...
export function admitDeadline(
  ...
  if (!Number.isFinite(maxInboundDeadlineMs) || maxInboundDeadlineMs <= 0) {
    throw new RangeError(
      "@nifrajs/core/budget: maxInboundDeadlineMs must be a finite positive number",
    )
  }
