...
const render = (props: Parameters<typeof Image>[0]): string =>
  renderToString(() => createComponent(Image, props)).toLowerCase()

describe("@nifrajs/web-solid/image", () => {
  ...
  test("forwards extra DOM props (class/id); never leaks nifra-only props to the DOM", () => {
    const html = render({
      src: "/x.png",
      width: 50,
      height: 50,
    ...
    expect(html).toContain('class="rounded') // Solid appends a trailing space to class
