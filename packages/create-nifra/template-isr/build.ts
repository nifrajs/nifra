// Build for Cloudflare Workers: (1) buildClient → public/assets (served by Workers Assets at
// /assets/*), and (2) buildServer → dist-server/worker.js (the ISR worker). `bun run build`.
...
console.log("worker bundle:", worker, "→ run: bunx wrangler dev")
