...
test("property: active link schemes never survive href normalization", () => {
  fc.assert(
...
test("property: mixed-case active schemes with embedded controls are always rejected", () => {
  const controls = fc.constantFrom("\u0000", "\t", "\n", "\r", " ", "\u001f", "\u007f")
  const activeHref = fc
    .tuple(fc.constantFrom("javascript:", "vbscript:"), fc.array(controls, { maxLength: 20 }))
    .map(([scheme, noise]) => {
