...
nifra migrate --from tailwind --to stylex  Dry-run safe static Tailwind → StyleX migration; --write applies it.
...
direct StyleX equivalent into `stylex.props(...)` plus a local `stylex.create(...)` table. Use
`--write` to apply the safe changes, and `--dir` to scan a project subdirectory. Responsive breakpoints
and element-local pseudo-classes are supported; dynamic expressions, arbitrary values, parent-dependent
variants, and unknown utilities are left untouched and reported with file and line evidence. A non-zero
exit status means manual migration issues remain.
