...
const policy = defineCapabilityPolicy({
  ...
  provenance: {
    imports: [
      { specifier: "@platform/db/read", capabilities: ["db.read"] },
      { specifier: "@platform/db/write", capabilities: ["db.write"] },
    ],
    forbiddenImports: [
      { specifier: "postgres", reason: "use the tenant-scoped @platform/db adapter" },
    ],
  },
})
...
describe("effect import scanner", () => {
  test("sees static, dynamic, require, and re-export imports but skips type-only imports", () => {
    expect(
      scanEffectImports(`
        import value from "@platform/db/read"
        import type { Row } from "@platform/db/types"
        const raw = await import("postgres")
        const x = require("@platform/db/write")
        export { helper } from "./helper"
        const templated = await import(\`@platform/db/read\`)
        // import(\`commented\`)
      `),
    ).toEqual([
      "@platform/db/read",
...
describe("project provenance firewall", () => {
  test("walks local imports, attributes effects conservatively, and reports raw bypasses", async () => {
    ...
    await writeFile(
      ...
      `import db from "postgres"
       import { write } from "@platform/db/write"
  ...
  test("explicit routeModules cover contract-style registrations", async () => {
    ...
    await mkdir(join(cwd, "src"), { recursive: true })
    await writeFile(join(cwd, "src/list.ts"), 'import "@platform/db/read"\n')
