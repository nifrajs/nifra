...
// The optimistic-UI demo's list. `id` is a stable surrogate so rows key by value, not array index.
...
export const backend = server()
  ...
  .get("/todos", () => ({ todos }))
  // Body validated at the trust boundary (length-capped to reject empty + oversized input).
