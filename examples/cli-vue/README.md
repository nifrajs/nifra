...
- **`nifra.config.ts`** — the CLI's build/dev tooling: `adapter` (re-exported), `clientModule`,
  ...
  the Bun-side SFC compile for `nifra build`/`dev` SSR), and Vue's `define` flags. This file
...
nifra build    # complete Bun deploy; Vue client + SSR routes are compiled into dist/
nifra start    # run the generated dist/server.js
```
