...
- `nifra.config.ts` - CLI-only `clientModule`. Compiler frameworks also put
  their `clientPlugins` / `serverPlugins` here.
- `backend.ts` - the typed contract (`export const backend`). Optional.

```sh
nifra dev      # true-HMR dev server (Bun native HMR + nifra SSR) - edit components/Counter.tsx live
nifra dev --vite  # opt into Vite middleware/HMR when needed
